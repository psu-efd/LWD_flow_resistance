...
def run_one_HEC_RAS_case_ManningN(case_ID, ManningN_MaterialID, ManningN, ManningN_MaterialName, faceless=True):
    """
    Run a single HEC-RAS case with the specified case_ID and ManningN value. The ManningN value is used to set the Manning's n for the LWD zone.
    ...
    print(f"Running HEC-RAS case {case_ID} with ManningN = {ManningN}")
    ...
    #HEC-RAS plan name: case_01, case_02, etc. (without the extension)
    plan_name = 'case_'+str(case_ID).zfill(2)
    ...
    #modify the Manning's n. In this example, we only modify one zone: LWD.
    ...
    shutil.copy(vtkFileNameList[-1], "../calibration_results/"+"case_"+str(case_ID).zfill(2)+"/RAS_2D_"+str(case_ID).zfill(2)+".vtk")
...
def sample_HEC_RAS_results(case_ID):
    ...
    #vtk file name
    vtk_file_name = "calibration_results/case_"+str(case_ID).zfill(2)+"/result.vtk"
...
def optimize_model_parameter(ManningN_or_Cd, parameter_bounds):
    ...
    result = gp_minimize(
        ...
        noise= (noise_level) ** 2,  # the noise variance of observation (5% estimated)newManningsNValues 
        random_state=1234)  # the random seed

    #print("result = ", result)

    # save gp_minimize result
    dump(result, 'calibration_results/case_'+str(current_case_ID).zfill(2)+'/gp_result.pkl')

    # save simulation results to files: 'simulation_results_'+str(current_case_ID).zfill(2)+'.dat' and 'calibrated_ManningN_'+str(current_case_ID).zfill(2)+'.dat'/'calibrated_Cd_'+str(current_case_ID).zfill(2)+'.dat'
    save_calibration_results(ManningN_or_Cd, result.x[0])
...
def objective_wrapper_function(x):
    ...
    if current_case_ID in [1, 2, 3, 4, 9, 10, 11, 12]:
        bFullWidth = True  #half width
    elif current_case_ID in [5, 6, 7, 8, 13, 14, 15, 16]:
    ...
    return objective_function_ManningN(x, current_case_ID, ManningN_MaterialID, ManningN_MaterialName, bFullWidth)

# Define the objective function to be minimized (for ManningN)
def objective_function_ManningN(ManningN, case_ID, ManningN_MaterialID, ManningN_MaterialName, bFullWidth):
    ...
    # Run the case with the current ManningN value. If the simulation is successful, the case_ID is returned; otherwise, -case_ID is returned.
    simulation_result = run_one_HEC_RAS_case_ManningN(case_ID, ManningN_MaterialID, ManningN[0], ManningN_MaterialName)
    ...
    measurement_file_name = '../measurement_data/measurement_results_case_'+str(current_case_ID).zfill(2)+'.dat'
...
def plot_calibration_history():
    ...
    simulation_results_file = 'calibration_results/case_'+str(current_case_ID).zfill(2)+'/simulation_results.dat'
    df_simulation_results = pd.read_csv(simulation_results_file, sep='\s+')

    ManningN_history = df_simulation_results['ManningN'].values
    ...
    plt.scatter(ManningN_history, Objective_function_history, label='Total error')

    #add labels and title
    plt.xlabel('ManningN', fontsize=16)
    ...
    plt.savefig("calibration_results/case_"+str(current_case_ID).zfill(2)+"/calibration_history.png", dpi=300, bbox_inches='tight', pad_inches=0.02)
...
def save_calibration_results(ManningN_or_Cd, calibrated_value):
    ...
    if ManningN_or_Cd == "ManningN":
        ...
        with open('calibration_results/case_'+str(current_case_ID).zfill(2)+'/simulation_results.dat', 'w') as f:
            f.write('ManningN error_total error_H error_Q_split H_upstream H_downstream Q_split\n')

            for ManningN, error_total, error_H, error_Q_split, H_upstream, H_downstream, Q_split in zip(
                    ManningN_history,
                ...
                sublist = [ManningN, error_total, error_H, error_Q_split, H_upstream, H_downstream, Q_split]
        ...
        with open('calibration_results/case_'+str(current_case_ID).zfill(2)+'/calibrated_ManningN.dat', 'w') as file:
            file.write(str(calibrated_value))


    elif ManningN_or_Cd == "Cd":
        with open('calibration_results/case_'+str(current_case_ID).zfill(2)+'/simulation_results.dat', 'w') as f:
            f.write('Cd error_total error_H error_Q_split H_upstream H_downstream Q_split\n')

            for Cd, error_total, error_H, error_Q_split, H_upstream, H_downstream, Q_split in zip(
                    Cd_history,
                    Objective_function_history,
                    Objective_function_history_Water_Depth,
                    Objective_function_history_Q_split,
                ...
                sublist = [Cd, error_total, error_H, error_Q_split, H_upstream, H_downstream, Q_split]

                # Join each sublist into a string with spaces separating elements
                f.write(' '.join(map(str, sublist)) + '\n')

        # write the calibrated Cd to result file
        with open('calibration_results/case_'+str(current_case_ID).zfill(2)+'/calibrated_Cd.dat', 'w') as file:
            file.write(str(calibrated_value))
...
def plot_optimization_results(ManningN_or_Cd, parameter_bounds):
    ...
    #read in 'gp_result_'+str(current_case_ID).zfill(2)+'.pkl' data
    result = load('calibration_results/case_'+str(current_case_ID).zfill(2)+'/gp_result.pkl')
    ...
    # save the figure to file
    plt.savefig("calibration_results/case_"+str(current_case_ID).zfill(2)+"/calibration.png", dpi=300, bbox_inches='tight', pad_inches=0.02)

    # Show the plot
    # plt.show()

def run_case_with_optimized_ManningN(case_ID):
    """
    run the case with the optimized ManningN
    """

    #read the calibrated parameter (Manning's n or Cd)
    calibrated_ManningN_file = 'calibration_results/case_'+str(case_ID).zfill(2)+'/calibrated_ManningN.dat'

    # Open the file and read the single float value
    with open(calibrated_ManningN_file, 'r') as file:
        ManningN = float(file.read().strip())  # Read and convert to float

        print("The calibrated ManningN is {}".format(ManningN))

    #run the case with the optimized ManningN
    ManningN_MaterialID = 1
    ManningN_MaterialName = 'LWD'
    simulation_result = run_one_HEC_RAS_case_ManningN(case_ID, ManningN_MaterialID, ManningN, ManningN_MaterialName, faceless=False)
