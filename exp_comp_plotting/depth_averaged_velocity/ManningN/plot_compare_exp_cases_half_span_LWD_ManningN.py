...
def plot_contour_from_vtk(case_ID, Fr, beta, ManningN, alpha_exp, alpha_simulation, rect_x, rect_y, rect_width, rect_height, vtkFileName,
    ...
    if case_ID in [5, 6, 7, 8]:
        ax.set_title(rf"Case {case_ID}: $Fr$ = {Fr:.3f}, half-span, with sediment, Manning's $n$ = {ManningN:.1f}, $\alpha_{{exp}}$ = {alpha_exp:.3f}, $\alpha_{{simulation}}$ = {alpha_simulation:.3f}", fontsize=52)
    else: 
        ax.set_title(rf"Case {case_ID}: $Fr$ = {Fr:.3f}, half-span, without sediment, Manning's $n$ = {ManningN:.1f}, $\alpha_{{exp}}$ = {alpha_exp:.3f}, $\alpha_{{simulation}}$ = {alpha_simulation:.3f}", fontsize=52)
...
if __name__ == "__main__":
    ...
    ManningN = [1.3,1.17,1.27,1.6,2.64,2.38,1.8,1.7]
