...
def plot_cases_combined(option):
    ...
    fig, axes = plt.subplots(8, 1, figsize=(10, 12), sharex=True, sharey=False,
    ...
    for i, exp_file in enumerate(sorted(exp_files)):
        ...
        # Create contour plot
        contourf = ax.contourf(X, Y, Z, levels=np.linspace(0, 0.5, 21), cmap='rainbow', alpha=0.8, norm=norm)
    ...
    save_filename = f"Cases_{range_start}_to_{range_end-1}_combined.pdf"
