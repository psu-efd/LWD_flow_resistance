...
#plot SRH-2D/HEC-RAS 2D simulation results + measurement velocity profiles
...
def plot_contour_from_vtk(case_ID, Fr, beta, ManningN_SRH_2D, ManningN_HEC_RAS_2D, alpha_exp, alpha_simulation, rect_x, rect_y, rect_width, rect_height, vtkFileName_SRH, vtkFileName_RAS,
                          U_all, x_positions, y_positions):

    #load data from vtk file: water depht and velocity

    if not os.path.exists(vtkFileName_SRH) or not os.path.exists(vtkFileName_RAS):
        return None

    points_SRH_2D, Vel_x_np_SRH_2D, Vel_y_np_SRH_2D, Vel_mag_np_SRH_2D = extract_data_from_vtk(vtkFileName_SRH, "SRH_2D")
    points_HEC_RAS_2D, Vel_x_np_HEC_RAS_2D, Vel_y_np_HEC_RAS_2D, Vel_mag_np_HEC_RAS_2D = extract_data_from_vtk(vtkFileName_RAS, "HEC_RAS_2D")
    ...
    if case_ID in [1, 2, 3, 4]:
        ax.set_title(rf"Case {case_ID}: $Fr$ = {Fr:.3f}, full-span, with sediment, Manning's $n$ = {ManningN_SRH_2D:.2f} (SRH-2D), {ManningN_HEC_RAS_2D:.2f} (HEC-RAS)", fontsize=52)
    else: 
        ax.set_title(rf"Case {case_ID}: $Fr$ = {Fr:.3f}, full-span, without sediment, Manning's $n$ = {ManningN_SRH_2D:.2f} (SRH-2D), {ManningN_HEC_RAS_2D:.2f} (HEC-RAS)", fontsize=52)
...
if __name__ == "__main__":
    ...
    ManningN_SRH_2D = [2.13, 2.16, 2.66, 2.72, 2.42, 2.36, 1.92, 1.98]
    ManningN_HEC_RAS_2D = [2.24, 2.24, 2.66, 2.72, 2.56, 2.42, 1.91, 1.93]
    ...
    for i, case_ID in enumerate(case_IDs):
        ...
        vtkFileName_SRH = "Exp_"+str(case_ID)+"_n/SRH2D_Exp_"+str(case_ID)+"_ManningN_C_0003.vtk"
        vtkFileName_RAS = "Exp_"+str(case_ID)+"_n/RAS_2D_"+str(case_ID).zfill(2)+".vtk"
        plot_contour_from_vtk(case_ID, Frs[i], betas[i], ManningN_SRH_2D[i], ManningN_HEC_RAS_2D[i], alphas_exp[i], alphas_simulation[i], rect_x, rect_y, rect_width, rect_height, vtkFileName_SRH, vtkFileName_RAS,
