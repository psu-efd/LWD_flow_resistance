...
def plot_contour_from_vtk(SRH_or_RAS, case_ID, Fr, beta, rect_x, rect_y, rect_width, rect_height, vtkFileName_mesh_1, vtkFileName_mesh_2, vtkFileName_mesh_3, vtkFileName_mesh_4, full_view_or_zoom_view):
    ...
    if SRH_or_RAS == "SRH_2D":
        if case_ID in [1, 2, 3, 4, 5, 6, 7, 8]: #with sediment
            if case_ID in [1, 2, 3, 4]: #full span
                fig.suptitle(f'Case {case_ID} with SRH-2D (Manning\'s $n$ approach): Fr = {Fr:.3f}, with sediment, full span', 
                            fontsize=14, y=0.98)
            else: #half span
                fig.suptitle(f'Case {case_ID} with SRH-2D (Manning\'s $n$ approach): Fr = {Fr:.3f}, with sediment, half span', 
                            fontsize=14, y=0.98)
        else: #no sediment
            if case_ID in [9, 10, 11, 12]: #full span
                fig.suptitle(f'Case {case_ID} with SRH-2D (Manning\'s $n$ approach): Fr = {Fr:.3f}, no sediment, full span', 
                            fontsize=14, y=0.98)
            else: #half span
                fig.suptitle(f'Case {case_ID} with SRH-2D (Manning\'s $n$ approach): Fr = {Fr:.3f}, no sediment, half span', 
                            fontsize=14, y=0.98)
    elif SRH_or_RAS == "HEC_RAS_2D":
        if case_ID in [1, 2, 3, 4, 5, 6, 7, 8]: #with sediment
            if case_ID in [1, 2, 3, 4]: #full span
                fig.suptitle(f'Case {case_ID} with HEC-RAS 2D (Manning\'s $n$ approach): Fr = {Fr:.3f}, with sediment, full span', 
                            fontsize=14, y=0.98)
            else: #half span
                fig.suptitle(f'Case {case_ID} with HEC-RAS 2D (Manning\'s $n$ approach): Fr = {Fr:.3f}, with sediment, half span', 
                            fontsize=14, y=0.98)
        else: #no sediment
            if case_ID in [9, 10, 11, 12]: #full span
                fig.suptitle(f'Case {case_ID} with HEC-RAS 2D (Manning\'s $n$ approach): Fr = {Fr:.3f}, no sediment, full span', 
                            fontsize=14, y=0.98)
            else: #half span
                fig.suptitle(f'Case {case_ID} with HEC-RAS 2D (Manning\'s $n$ approach): Fr = {Fr:.3f}, no sediment, half span', 
                            fontsize=14, y=0.98)
    ...
    # Save the plot
    if full_view_or_zoom_view == "zoom_view":
        output_filename = f'mesh_effect_case_{case_ID}_ManningN_{SRH_or_RAS}_zoom_view.png'
    else:
        output_filename = f'mesh_effect_case_{case_ID}_ManningN_{SRH_or_RAS}.png'
...
if __name__ == "__main__":
    ...
    ManningN_SRH_2D = [2.72, 1.5, 1.98, 1.62]
    ManningN_SRH_2D_coarse_1 = [2.13, 2.16, 2.66, 2.72]
    ManningN_SRH_2D_coarse_2 = [2.13, 2.16, 2.66, 2.72]
    ManningN_SRH_2D_coarse_3 = [2.13, 2.16, 2.66, 2.72]
    
    ManningN_HEC_RAS_2D = [2.72, 1.49, 1.93, 1.84]
    ManningN_HEC_RAS_2D_coarse_1 = [2.72, 1.49, 1.93, 1.84]
    ManningN_HEC_RAS_2D_coarse_2 = [2.72, 1.49, 1.93, 1.84]
    ManningN_HEC_RAS_2D_coarse_3 = [2.72, 1.49, 1.93, 1.84]
    ...
    for i, case_ID in enumerate(case_IDs):
        ...
        #plot SRH-2D results
        vtkFileName_mesh_1 = "Exp_"+str(case_ID)+"_n/SRH2D_Exp_"+str(case_ID)+"_ManningN_C_0003.vtk"
        vtkFileName_mesh_2 = "Exp_"+str(case_ID)+"_n/SRH2D_Exp_"+str(case_ID)+"_ManningN_C_0003_coarse_1.vtk"
        vtkFileName_mesh_3 = "Exp_"+str(case_ID)+"_n/SRH2D_Exp_"+str(case_ID)+"_ManningN_C_0003_coarse_2.vtk"
        vtkFileName_mesh_4 = "Exp_"+str(case_ID)+"_n/SRH2D_Exp_"+str(case_ID)+"_ManningN_C_0003_coarse_3.vtk"
        ...
        vtkFileName_mesh_1 = "Exp_"+str(case_ID)+"_n/RAS_2D_"+str(case_ID).zfill(2)+".vtk"
        vtkFileName_mesh_2 = "Exp_"+str(case_ID)+"_n/RAS_2D_"+str(case_ID).zfill(2)+"_coarse_1.vtk"
        vtkFileName_mesh_3 = "Exp_"+str(case_ID)+"_n/RAS_2D_"+str(case_ID).zfill(2)+"_coarse_2.vtk"
        vtkFileName_mesh_4 = "Exp_"+str(case_ID)+"_n/RAS_2D_"+str(case_ID).zfill(2)+"_coarse_3.vtk"
