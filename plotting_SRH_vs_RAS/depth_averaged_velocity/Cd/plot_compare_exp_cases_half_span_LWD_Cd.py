...
def plot_contour_from_vtk(case_ID, Fr, beta, Cd_SRH_2D, Cd_HEC_RAS_2D, alpha_exp, alpha_simulation_SRH_2D, alpha_simulation_HEC_RAS_2D, rect_x, rect_y, rect_width, rect_height, vtkFileName_SRH_2D, vtkFileName_HEC_RAS_2D, 
                          U_all, V_all, x_positions, y_positions):

    #load data from vtk file: water depht and velocity

    if not os.path.exists(vtkFileName_SRH_2D) or not os.path.exists(vtkFileName_HEC_RAS_2D):
        print("vtk file not found: ", vtkFileName_SRH_2D, " or ", vtkFileName_HEC_RAS_2D)
        return None
    ...
    Z_RAS_2D = griddata(points_HEC_RAS_2D, z_RAS_2D, (X, Y), method="linear")
    ...
    for iProfile in range(5):
        ...
        interpolated_HEC_RAS_2D_velocity_x = griddata((x_RAS_2D, y_RAS_2D), Vel_x_np_HEC_RAS_2D, coords_at_points, method='linear')
        interpolated_HEC_RAS_2D_velocity_y = griddata((x_RAS_2D, y_RAS_2D), Vel_y_np_HEC_RAS_2D, coords_at_points, method='linear')
        ...
        for iPoint in range(nPoints):
            ...
            #plot HEC-RAS velocity
            plt.arrow(coords_at_points[iPoint, 0], coords_at_points[iPoint, 1],
                      interpolated_HEC_RAS_2D_velocity_x[iPoint], interpolated_HEC_RAS_2D_velocity_y[iPoint],
        ...
        #print("dot_x_positions1:", dot_x_positions1)
        ...
        #dot_y_positions1 = [y_positions[iProfile] + value + 0.01 for value in V_all[iProfile]]
    ...
    if case_ID in [5, 6, 7, 8]:
        ax.set_title(rf"Case {case_ID}: $Fr$ = {Fr:.3f}, half-span, with sediment, $C_d$ = {Cd_SRH_2D:.1f} (SRH-2D), {Cd_HEC_RAS_2D:.1f} (HEC-RAS)", fontsize=52)
    else: 
        ax.set_title(rf"Case {case_ID}: $Fr$ = {Fr:.3f}, half-span, without sediment, $C_d$ = {Cd_SRH_2D:.1f} (SRH-2D), {Cd_HEC_RAS_2D:.1f} (HEC-RAS)", fontsize=52)
    ...
    fig.savefig("exp_vel_mag_contour_Cd_"+ str(case_ID).zfill(4) +".png", dpi=300, bbox_inches='tight', pad_inches=0)
...
if __name__ == "__main__":
    ...
    Cds_SRH_2D = [16.4, 14, 20.3, 25.4, 64.2, 53.2, 43.8, 38.1]
    Cds_HEC_RAS_2D = [15.6, 12, 18.48, 22.68, 59.8, 49.84, 41.6, 35]
    ...
    alphas_simulation_HEC_RAS_2D = [0.778,	0.771,	0.823,	0.831,	0.867,	0.858,	0.846, 0.833]
    ...
    for i, case_ID in enumerate(case_IDs):
        ...
        vtkFileName_SRH_2D = "Exp_"+str(case_ID)+"_Cd/SRH2D_Exp_"+str(case_ID)+"_Cd_C_0003.vtk"
        vtkFileName_HEC_RAS_2D = "Exp_"+str(case_ID)+"_Cd/RAS_2D_"+str(case_ID).zfill(2)+".vtk"

        plot_contour_from_vtk(case_ID, Frs[i], betas[i], Cds_SRH_2D[i], Cds_HEC_RAS_2D[i], alphas_exp[i], alphas_simulation_SRH_2D[i], alphas_simulation_HEC_RAS_2D[i], rect_x, rect_y, rect_width, rect_height, vtkFileName_SRH_2D, vtkFileName_HEC_RAS_2D, [U1s[i], U2s[i], U3s[i], U4s[i], U5s[i]], [V1s[i], V2s[i], V3s[i], V4s[i], V5s[i]], x_positions, y_positions)
