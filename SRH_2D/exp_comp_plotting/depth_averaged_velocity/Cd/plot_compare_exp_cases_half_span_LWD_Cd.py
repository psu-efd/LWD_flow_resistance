...
def plot_contour_from_vtk(case_ID, Fr, beta, Cd, alpha_exp, alpha_simulation, rect_x, rect_y, rect_width, rect_height, vtkFileName, 
    ...
    if case_ID in [5, 6, 7, 8]:
        ax.set_title(rf"Case {case_ID}: $Fr$ = {Fr:.3f}, half-span, with sediment, $C_d$ = {Cd:.1f}, $\alpha_{{exp}}$ = {alpha_exp:.3f}, $\alpha_{{simulation}}$ = {alpha_simulation:.3f}", fontsize=52)
    else: 
        ax.set_title(rf"Case {case_ID}: $Fr$ = {Fr:.3f}, half-span, without sediment, $C_d$ = {Cd:.1f}, $\alpha_{{exp}}$ = {alpha_exp:.3f}, $\alpha_{{simulation}}$ = {alpha_simulation:.3f}", fontsize=52)
...
if __name__ == "__main__":
    ...
    Cds = [16.5, 14.1, 20.5, 25.6, 64.8, 53.4, 45.5, 40]
    betas = [0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5]
    alphas_exp = [0.779, 0.775,	0.824, 0.832, 0.868, 0.857, 0.846, 0.836]
    alphas_simulation = [0.779,	0.777,	0.823,	0.831,	0.868,	0.856,	0.846, 0.837]
    ...
    for i, case_ID in enumerate(case_IDs):
        ...
        vtkFileName = "Exp_"+str(case_ID)+"_Cd/SRH2D_Exp_"+str(case_ID)+"_Cd_C_0003"".vtk"
        plot_contour_from_vtk(case_ID, Frs[i], betas[i], Cds[i], alphas_exp[i], alphas_simulation[i], rect_x, rect_y, rect_width, rect_height, vtkFileName,
                              [U1s[i], U2s[i], U3s[i], U4s[i], U5s[i]], [V1s[i], V2s[i], V3s[i], V4s[i], V5s[i]], x_positions, y_positions)
