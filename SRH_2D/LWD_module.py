...
def clear_history():
    """
    Clear the history of the calibration results.
    """
    Cd_history.clear()
    ...
    WSE_p1_history.clear()
    WSE_p2_history.clear()
    WSE_p3_history.clear()
    H_p1_history.clear()
    H_p2_history.clear()
    H_p3_history.clear()
    Q_Line2_history.clear()
    Q_Line3_history.clear()
...
def run_srh_2d_model_with_given_Cd(Cd, destination_folder='case_temp'):
    ...
    #monitoring point 1
    df = pd.read_csv("Output_MISC/" + case_name + "_PT1.dat", sep='\s+')
    # Access the value in the last row and the fifth column (WSE)
    Zb_p1 = df.iloc[-1, 3]  # bed elevation
    WSE_p1 = df.iloc[-1, 4]  #WSE
    H_p1 = df.iloc[-1, 5]    #water depth
    ...
    # monitoring point 2
    df = pd.read_csv("Output_MISC/" + case_name + "_PT2.dat", sep='\s+')
    # Access the value in the last row and the fifth column (WSE)
    Zb_p2 = df.iloc[-1, 3]  # bed elevation
    WSE_p2 = df.iloc[-1, 4]
    H_p2 = df.iloc[-1, 5]
    ...
    # monitoring point 3
    df = pd.read_csv("Output_MISC/" + case_name + "_PT3.dat", sep='\s+')
    # Access the value in the last row and the fifth column (WSE)
    Zb_p3 = df.iloc[-1, 3]  # bed elevation
    WSE_p3 = df.iloc[-1, 4]
    H_p3 = df.iloc[-1, 5]
    ...
    # monitoring line 2
    df = pd.read_csv("Output_MISC/" + case_name + "_LN2.dat", sep='\s+')
    # Access the value in the last row and the second column (Q)
    Q_Line2 = abs(df.iloc[-1, 1])   #get the absolute value

    # monitoring line 3
    df = pd.read_csv("Output_MISC/" + case_name + "_LN3.dat", sep='\s+')
    # Access the value in the last row and the second column (Q)
    Q_Line3 = abs(df.iloc[-1, 1])   #get the absolute value
