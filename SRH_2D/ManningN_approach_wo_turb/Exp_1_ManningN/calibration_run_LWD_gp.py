...
def objective_with_fixed_boolean(x):
    ...
    bFullWidth = True  #don't forget to change this value (full or half width)

    return LWD_module.objective_function_ManningN(x, bFullWidth)

if __name__ == "__main__":

    #Manning's n or Cd
    ManningN_or_Cd = "ManningN"

    # Define the bounds for the parameter Manning's n
    ManningN_bounds = [(1.5, 3.0)]

    #clean the global lists
    LWD_module.clear_history()

    #optimize SRH-2D model parameter (Manning's n or Cd), and save result to "gp_result.pkl".
    optimize_model_parameter(ManningN_or_Cd, ManningN_bounds)

    #plot optimization results
    LWD_module.plot_optimization_results(ManningN_or_Cd, ManningN_bounds)
